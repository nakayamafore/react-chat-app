...
const loadLocalStorage = key => key && JSON.parse(localStorage.getItem(key))
...
const convertMarkdownUploadFileUrl = e => {
    ...
    if (img_exts.ext.includes(ext)) {
        ...
        if (targetExt) {
            ...
            return `[![${e.fileName}](${txt_exts.icon})](${e.fileUrl})\n\n*${e.fileName}*`
        }
    }
}
const uploadBlobImage = async (event, setUploadfile, setContent) => {
    const imageType = event.types.find(t => t === "image/png")
    if (!imageType) return

    const imageBlob = await event.getType(imageType)
    const reader = new FileReader()
    reader.readAsDataURL(imageBlob)
    reader.onload = function () {
        const token = loadLocalStorage("token")
        const method = "POST";
        const headers = { 'Accept': 'application/json', 'Content-Type': 'application/json', 'x-jwt-token': token }
